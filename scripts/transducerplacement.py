...
if '/Applications/SimNIBS-4.1/simnibs_env/lib/python3.9/site-packages' not in sys.path:
    sys.path.append('/Applications/SimNIBS-4.1/simnibs_env/lib/python3.9/site-packages')
...
t1_filepath = config['paths']['T1w']

subject_id = config.get('subject_id', 'default_subject')

# Path to head mesh (.msh file) generated by SimNIBS' charm
simnibs_mesh_filepath = os.path.join(config['paths']['simnibs_directory'], f'm2m_{subject_id}', f'{subject_id}.msh')

# Name of and path to mask of target region of interest (in same space as T1
# image)
target_roi_name = config['freesurfer']['roi']['name']
target_roi_filepath = os.path.join(config['paths']['output_directory'], f"{config['freesurfer']['roi']['name']}_mask.nii.gz")

# Maximum focal depth of transducer (in mm)
# Note: Set to 1000 if you want a "whole-head" map of distances
max_distance = config['transducer_placement']['max_distance'] # note: max. focal depth of CTX-500 ##

# Maximum allowed angle for tilting of TUS transducer (in degree)
# Note: Set to 360 if you want a "whole-head" map of angles
max_angle = config['transducer_placement']['max_angle'] 
...
#for f in glob.glob(simnibs_mesh_path + "/skin*.nii.gz"):
    #os.remove(f)
#for f in glob.glob(simnibs_mesh_path + "/final_tissues*"):
    #os.remove(f)
...
#=============================================================================
# Add structure information to .gii files and open in Connectome Workbench
#=============================================================================

for f in glob.glob(simnibs_mesh_path + "/*.gii"):
